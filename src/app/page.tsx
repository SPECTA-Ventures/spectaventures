...
const CONTACT_EMAIL = "ventures.specta@gmail.com";
...
export default function Page() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!json?.ok) throw new Error("Failed to send message");
  ...
  return (
    <div className="bg-black text-white min-h-screen flex flex-col">
        ...
        {!sent ? (
          <form className="grid gap-6 text-left" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={sending}
              className="bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-lg py-4 rounded-2xl shadow-xl"
            >
      ...
      <footer className="bg-black py-10 text-center text-gray-500 text-sm">
        <p>© {new Date().getFullYear()} Specta Marketing Agency. All rights reserved.</p>
      </footer>
    </div>
  );
}
